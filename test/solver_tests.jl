...
using SimpleSolvers: NewtonSolver, NonlinearSolverStatus, SolverState, isconverged,
                     solve_with_status!
...
@test default_linesearch(TestMethod) isa Backtracking{Float64}
@test which(default_linesearch, Tuple{Type{TestMethod}}) !==
      which(default_linesearch, Tuple{Type{Float32}})
...
@test which(default_linesearch, Tuple{Type{Rational{Int}}}) ===
      which(default_linesearch, Tuple{Type{Float32}})
...
let
    ...
    y = [1.0]
    s2 = NewtonSolver(
        y, similar(y); F = F!, verbosity = 0, max_iterations = 1, min_iterations = 1)
